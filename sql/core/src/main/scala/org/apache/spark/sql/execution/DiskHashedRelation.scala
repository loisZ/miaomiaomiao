...
private[sql] object DiskHashedRelation {
  ...
  /**
    ...
    * @param size he number of [[DiskPartition]]s
  ...
  def apply (
    ...
    while(input.hasNext){
      ...
      val t_index: Int = keyGenerator(row).hashCode() % size
      partitionArray(t_index).insert(row)
    }
    ...
    val generalDiskHR = new GeneralDiskHashedRelation(partitionArray)
    generalDiskHR.closeAllPartitions()
    generalDiskHR
  }
}
